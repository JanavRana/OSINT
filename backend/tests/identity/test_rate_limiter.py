...
def test_get_available_tokens(rate_limiter):
    ...
    # Use approximate comparison to handle floating-point precision and minor token refill
    available = rate_limiter.get_available_tokens("test-plugin")
    assert abs(available - 7) < 0.01  # Allow small tolerance for timing/precision
